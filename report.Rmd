...
samewiki_ctr <- searches %>% # save object samewiki_ctr for odds ratio plot
...
```{r max_clicked, echo=FALSE, fig.height=6*length(unique(events$wiki))}
searches %>%
...
### PaulScore
```{r paulscores, echo=FALSE, fig.height=6*ceiling(length(unique(events$wiki))/3)}
searches %>%
  dplyr::ungroup() %>%
...
```{r search_offset, eval = exists("serp_offset"), echo = FALSE, fig.height=6*ceiling(length(unique(events$wiki))/3)}
serp_offset %>%
...
```{r dwelltime, eval=exists("visitedPages"), echo=FALSE, fig.height=6*length(unique(events$wiki))}
temp <- visitedPages
...
```{r scroll, eval=exists("visitedPages"), echo=FALSE, fig.height=6*ceiling(length(unique(events$wiki))/3)}
visitedPages %>%
  dplyr::group_by(group, wiki) %>%
