...
```{r ssclick_summary, eval = exists("ssclick_des"), echo = FALSE, fig.height=6*length(unique(events$wiki))}
ssclick_des %>%
...
```{r hover_breakdown, eval = exists("hover_over"), echo = FALSE, fig.height=6*length(unique(events$wiki))}
hover_over %>%
  dplyr::left_join(events, by = c("session_id", "event_id")) %>%
  dplyr::filter(!is.na(section) & !is.na(results)) %>%
  dplyr::mutate(results = ifelse(results >= 5, "5+ result(s)", paste(results, "result(s)"))) %>%
