...
```{r sql_setup, echo=FALSE, message=FALSE}
...
## Data Cleansing
```{r data_cleansing, echo =FALSE, warning=FALSE, results='asis'}
...
```{r data_wrangling, echo=FALSE, warning=FALSE}
# Aggregate by search
searches <- events %>%
  ...
  dplyr::summarize(timestamp = timestamp[1],
                   `got same-wiki results` = any(`some same-wiki results`, na.rm = TRUE),
...
if ("searchResultPage" %in% events$event){
  ...
  serp_offset <- events %>%
    ...
    mutate(offset = unlist(lapply(.$event_extraParams, function(x) parse_extraParams(x, action = "searchResultPage")$offset))) %>%
...
if ("esclick" %in% events$event){
  esclick_result <- events %>%
    filter(event == "esclick") %>%
    cbind(., do.call(bind_rows, lapply(.$event_extraParams, parse_extraParams, action =  "esclick"))) %>%
...
if (all(c("hover-on", "hover-off") %in% events$event)){
  hover_over <- events %>%
    filter(event %in% c("hover-on", "hover-off")) %>%
    cbind(., do.call(bind_rows, lapply(.$event_extraParams, parse_extraParams, action =  c("hover-on", "hover-off")))) %>%
...
`r if(exists("serp_offset")){"### Number of SERP events by offset"}`
```{r serp_offset_summary, eval = exists("serp_offset"), echo = FALSE}
serp_offset_summary <- serp_offset %>%
  dplyr::filter(!is.na(offset)) %>%
  dplyr::group_by(offset) %>%
  tally %>%
  dplyr::mutate(offset = paste(offset, "result(s)"))
output <- serp_offset_summary$n
names(output) <- serp_offset_summary$offset
data.frame(`No.Events` = output) %>% t() %>% knitr::kable()
rm(serp_offset_summary, output)
```

`r if(exists("serp_iw")){"### Number of SERP events by source and position for sister-search sidebar results"}`
```{r serp_iw_summary, eval = exists("serp_iw"), echo = FALSE}
serp_iw %>%
  dplyr::filter(!is.na(source) & !is.na(position)) %>%
  dplyr::mutate(position = paste(safe_ordinals(position), "result")) %>%
...
`r if(exists("ssclick_des")){"### Number of sister search clicks by language and project"}`
```{r ssclick_summary, eval = exists("ssclick_des"), echo = FALSE}
ssclick_des %>%
  dplyr::group_by(language, project) %>%
  dplyr::tally() %>%
  xtabs(n ~ language + project, data = .) %>%
  addmargins %>%
  knitr::kable()
```
...
esclick_result %>%
  dplyr::filter(!is.na(section) & !is.na(result)) %>%
  dplyr::mutate(result = paste(safe_ordinals(result + 1), "result")) %>%
...
```{r hover_breakdown, eval = exists("hover_over"), echo = FALSE}
hover_breakdown <- hover_over %>%
  dplyr::filter(event == "hover-on") %>%
  dplyr::group_by(section, results) %>%
  dplyr::summarise(counts = n()) %>%
  xtabs(counts ~ section + results, data = .) %>%
  addmargins
colnames(hover_breakdown) <- ifelse(colnames(hover_breakdown) == "Sum", "Sum", paste(colnames(hover_breakdown), "result(s)"))
hover_breakdown %>% knitr::kable()
```
